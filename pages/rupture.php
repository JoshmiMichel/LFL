...
?>
<?php
   if(isset($_POST['design']) AND isset($_POST['prixU']) AND isset($_POST['qteAchete']) AND isset($_POST['montant1'])){
       try{
        $pdo_options[PDO::ATTR_ERRMODE]=PDO::ERRMODE_EXCEPTION;
        $bdd= new PDO('mysql: host=localhost ; dbname=vente_provande','root','',$pdo_options);
        $req = $bdd->prepare('INSERT INTO panier ( nomP , prixP  , qte ,  montant ,  dateP ) VALUES(?,
        ?, ?, ?, NOW())');
        $req->execute(array($_POST['design'] ,$_POST['prixU'], $_POST['qteAchete'] , $_POST['montant1'] ) );
      }catch(Exception $e){
          die('ERREUR : '.$e->getMessage());
      }
    }
